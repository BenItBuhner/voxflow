      ...
      data: [
        {
          ...
          price: { id: price, recurring: { interval: price === 'price_year' ? 'year' : 'month' } }
        }
      ]
    },
    ...overrides
...
async function deliver(
  ...
  return await t.fetch('/stripe/webhook', {
    ...
    headers: {
      ...
      'stripe-signature': header ?? (await sign(payload, STRIPE_ENV.STRIPE_WEBHOOK_SECRET))
    },
    body: payload
  })
}

const decode = (body: BodyInit | null | undefined): URLSearchParams =>
...
describe('stripe helpers', () => {
  it('reads the configuration and knows when billing can run', () => {
    ...
    expect(
      readStripeConfig({ ...STRIPE_ENV, MURMUR_SITE_URL: '', STRIPE_WEBHOOK_SECRET: '' })
    ).toMatchObject({ webhookSecret: null, siteUrl: null })
  ...
  it('encodes nested form parameters the way Stripe expects', () => {
    expect(
      formEncode({
        mode: 'subscription',
        line_items: { 0: { price: 'price_x', quantity: 1 } },
        skip: undefined,
        metadata: { clerkId: 'u 1' }
      })
    ).toBe(
      'mode=subscription&line_items%5B0%5D%5Bprice%5D=price_x&line_items%5B0%5D%5Bquantity%5D=1&metadata%5BclerkId%5D=u%201'
    )
    const checkout = checkoutSessionParams(readStripeConfig(STRIPE_ENV)!, {
      clerkId: 'user_ada',
    ...
    expect(params.get('success_url')).toBe(
      `${SITE}/account?checkout=success&session_id={CHECKOUT_SESSION_ID}`
    )
    ...
    const returning = decode(
      formEncode(
        checkoutSessionParams(readStripeConfig(STRIPE_ENV)!, {
          clerkId: 'user_ada',
          email: 'ada@example.com',
          customerId: 'cus_ada',
          interval: 'month'
    ...
    expect(
      decode(formEncode(portalSessionParams(readStripeConfig(STRIPE_ENV)!, 'cus_ada'))).get(
        'return_url'
      )
    ).toBe(`${SITE}/account`)
  })
  ...
  it('verifies webhook signatures within the tolerance window', async () => {
    ...
    await expect(verifyStripeSignature(payload, header, 'whsec_b', NOW)).rejects.toThrow(
      StripeSignatureError
    )
    await expect(verifyStripeSignature(payload + ' ', header, 'whsec_a', NOW)).rejects.toThrow(
      /does not match/
    )
    await expect(verifyStripeSignature(payload, header, 'whsec_a', NOW + 301_000)).rejects.toThrow(
      /tolerance/
    )
    await expect(verifyStripeSignature(payload, null, 'whsec_a', NOW)).rejects.toThrow(/Missing/)
    await expect(verifyStripeSignature(payload, 't=abc,v1=00', 'whsec_a', NOW)).rejects.toThrow(
      /Malformed/
    )
    // Stripe may send several v1 signatures during a secret rotation; one match is enough.
    await expect(
      verifyStripeSignature(payload, `${header},v1=deadbeef`, 'whsec_a', NOW)
  ...
  it('reads subscriptions in both API shapes and maps statuses to entitlement', () => {
    ...
    const legacy = subscriptionSnapshot(
      {
        ...
        items: { data: [{ price: { id: 'price_other', recurring: { interval: 'month' } } }] }
      },
      PRICES
    )
    ...
    expect(
      subscriptionSnapshot({
        id: 'sub_3',
        status: 'active',
        items: { data: [{ price: { id: 'p', recurring: { interval: 'week' } } }] }
      })
    ).toBeNull()
    ...
    for (const status of [
      ...
      expect(entitlesPro(status)).toBe(false)
  })
  ...
  it('reduces the webhook events Murmur acts on', () => {
    ...
    expect(
      parseStripeEvent(JSON.parse(stripeEvent('checkout.session.completed', { mode: 'payment' })))
    ).toMatchObject({ type: 'ignored', reason: 'not a subscription' })
    expect(
      parseStripeEvent(
        JSON.parse(stripeEvent('customer.subscription.updated', stripeSubscription())),
        PRICES
      )
    ).toMatchObject({
      type: 'customer.subscription.updated',
      subscription: { id: 'sub_1', interval: 'year' }
    })
    expect(
      parseStripeEvent(
        JSON.parse(stripeEvent('customer.subscription.deleted', { id: 'sub_1' })),
        PRICES
      )
    ).toMatchObject({ type: 'ignored', reason: 'unreadable subscription' })
    expect(
      parseStripeEvent(
        JSON.parse(
          stripeEvent('invoice.payment_failed', { customer: 'cus_ada', subscription: 'sub_1' })
        )
      )
    ).toMatchObject({
    ...
    ).toMatchObject({ subscriptionId: 'sub_9' })
    expect(parseStripeEvent(JSON.parse(stripeEvent('customer.created', {})))).toEqual({
      type: 'ignored',
...
describe('billing without Stripe configured', () => {
  it('degrades: status says so, the actions refuse readably, the webhook is off', async () => {
    ...
    await expect(asAda.action(api.billing.createPortalSession, {})).rejects.toThrow(
      /not switched on/
    )
    await expect(t.action(api.billing.createCheckoutSession, { interval: 'year' })).rejects.toThrow(
      /Not authenticated/
    )
    const error = vi.spyOn(console, 'error').mockImplementation(() => undefined)
    expect(
      (await deliver(t, stripeEvent('customer.subscription.updated', stripeSubscription()))).status
    ).toBe(500)
    error.mockRestore()
  })
})
...
describe('billing with Stripe configured', () => {
  ...
  it('turns a completed Checkout into Pro by reading the subscription back from Stripe', async () => {
    const calls = stubFetch((url) =>
      url.endsWith('/v1/subscriptions/sub_1')
        ? jsonResponse(stripeSubscription())
        : jsonResponse({}, 404)
    )
    const t = setup()
    const asAda = t.withIdentity(ada)
  ...
  it('follows the subscription through payment failure, recovery, cancellation and deletion', async () => {
    ...
    expect(
      (
        await deliver(
          t,
          stripeEvent('customer.subscription.updated', stripeSubscription(), created)
        )
      ).status
    ).toBe(200)
      ...
      (
        await deliver(
          ...
          stripeEvent(
            ...
            { customer: 'cus_ada', subscription: 'sub_1' },
            created + 10
          )
        )
      ).status
    ...
    expect(billing.subscription).toMatchObject({
      status: 'active',
      paymentFailedAt: (created + 10) * 1000
    })
    expect(await asAda.query(api.users.me, {})).toMatchObject({ planState: 'pro' })
    ...
    await deliver(
      t,
      stripeEvent('customer.subscription.updated', stripeSubscription(), created + 30)
    )
    expect((await asAda.query(api.billing.status, {})).subscription).toMatchObject({
      status: 'active',
      paymentFailedAt: null
    })
    ...
    await deliver(
      ...
      stripeEvent(
        'customer.subscription.deleted',
        stripeSubscription({ id: 'sub_2', status: 'canceled' }),
        created + 70
      )
    )
    ...
    await deliver(
      ...
      stripeEvent(
        'customer.subscription.updated',
        stripeSubscription({ id: 'sub_3', status: 'unpaid' }),
        created + 90
      )
    )
  ...
  it('ignores events for a replaced subscription and finds accounts by customer', async () => {
    ...
    await deliver(
      t,
      stripeEvent('customer.subscription.updated', stripeSubscription({ id: 'sub_old' }), created)
    )
    await deliver(
      t,
      stripeEvent(
        'customer.subscription.updated',
        stripeSubscription({ id: 'sub_new' }),
        created + 1
      )
    )
    ...
    await deliver(
      ...
      stripeEvent(
        'customer.subscription.updated',
        stripeSubscription({ id: 'sub_new', status: 'canceled', metadata: {} }),
        created + 3
      )
    )
    ...
    expect(
      (
        await deliver(
          ...
          stripeEvent(
            'customer.subscription.updated',
            stripeSubscription({ id: 'sub_x', customer: 'cus_nobody', metadata: {} }),
            created + 4
          )
        )
      ).status
