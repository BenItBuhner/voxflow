...
const snapshot = (days: DayUsage[] = [], month: Partial<UsageSnapshot['month']> = {}): UsageSnapshot => ({
  day: TODAY,
  days,
  month: { sttSeconds: 0, sttRequests: 0, llmTokens: 0, llmRequests: 0, ...month }
})

const free = { plan: 'free' as const, state: 'free' as const, limits: PLANS.free, snapshot: snapshot(), now: NOW }
const pro = { plan: 'pro' as const, state: 'pro' as const, limits: PLANS.pro, snapshot: snapshot(), now: NOW }

type T = ReturnType<typeof setup>

async function seedDay(t: T, userId: Id<'users'>, day: string, partial: Partial<DayUsage>): Promise<void> {
...
async function seedMonth(
  t: T,
  userId: Id<'users'>,
  partial: Partial<{ sttSeconds: number; llmTokens: number; windowStart: number; windowCount: number }>
): Promise<void> {
  await t.run(async (ctx) => {
...
describe('calendar helpers', () => {
  ...
  it('says when a rolling meter next moves', () => {
    ...
    const old = weekBuckets(TODAY, [bucket('2026-09-07', { words: 300 }), bucket('2026-09-11', { words: 250 })])
...
describe('meters', () => {
  it('lists the free tier’s meters in display order with their resets', () => {
    const meters = metersFor(
      PLANS.free,
      snapshot([bucket('2026-09-09', { words: 200, sttSeconds: 100, dictations: 5 }), bucket(TODAY, { dictations: 2, formats: 4 })], {
        sttSeconds: 300,
        llmTokens: 1000
      })
    )
    ...
    expect(meters[0]).toEqual({ limit: 'wordsPerWeek', used: 200, allowed: 500, exceeded: false, resetsAt: Date.UTC(2026, 8, 16) })
    ...
    expect(meters[2]).toEqual({ limit: 'dictationsPerDay', used: 4, allowed: 12, exceeded: false, resetsAt: TOMORROW })
    expect(meters[3]).toEqual({ limit: 'sttSecondsPerMonth', used: 300, allowed: 7200, exceeded: false, resetsAt: OCTOBER })
  ...
  it('gives Pro the fair-use meters and no weekly ones', () => {
    ...
    expect(meters.map((m) => m.limit)).toEqual(['fairUseSttSecondsPerMonth', 'sttSecondsPerMonth', 'llmTokensPerMonth'])
    expect(meters[0]).toEqual({ limit: 'fairUseSttSecondsPerMonth', used: 108_000, allowed: 108_000, exceeded: true, resetsAt: OCTOBER })
...
describe('request checks', () => {
  it('refuses clips longer than the tier allows, with the tier’s cap', () => {
    expect(checkTranscription(free, 61)).toMatchObject({
      ...
      allowed: 60,
      resetsAt: null,
      message: 'Clips longer than 1 minute cannot be sent on the free plan; Pro takes clips up to 10 minutes.'
    })
    expect(checkTranscription(free, 60)).toBeNull()
    expect(checkTranscription(pro, 61)).toBeNull()
    expect(checkTranscription(pro, 601)).toMatchObject({ status: 413, allowed: 600, message: /10 minutes cannot be sent to Murmur/ })
  })

  it('stops the free tier at its word, audio, daily and monthly caps in that order', () => {
    const words = checkTranscription({ ...free, snapshot: snapshot([bucket('2026-09-08', { words: 500 })]) }, 10)
    expect(words).toMatchObject({
      ...
      message: "You've dictated the 500 words a week the free plan includes; more open up on Tue, Sep 15, or go unlimited with Pro."
    })
    // 499 words: still allowed, however long the clip within the other caps.
    expect(checkTranscription({ ...free, snapshot: snapshot([bucket(TODAY, { words: 499 })]) }, 30)).toBeNull()

    const audio = checkTranscription({ ...free, snapshot: snapshot([bucket(TODAY, { sttSeconds: 400 })]) }, 30)
    expect(audio).toMatchObject({ limit: 'sttSecondsPerWeek', used: 400, allowed: 420, resetsAt: Date.UTC(2026, 8, 20) })
    expect(audio?.message).toBe('The free plan transcribes 7 minutes of audio a week; that is used up until Sun, Sep 20. Pro has no weekly limit.')
    expect(checkTranscription({ ...free, snapshot: snapshot([bucket(TODAY, { sttSeconds: 400 })]) }, 20)).toBeNull()

    const day = checkTranscription({ ...free, snapshot: snapshot([bucket(TODAY, { dictations: 12 })]) }, 5)
    expect(day).toMatchObject({ limit: 'dictationsPerDay', used: 12, allowed: 12, resetsAt: TOMORROW, retryAfterSec: 43_200 })
    // Yesterday's dictations do not count today.
    expect(checkTranscription({ ...free, snapshot: snapshot([bucket('2026-09-12', { dictations: 12 })]) }, 5)).toBeNull()

    const month = checkTranscription({ ...free, snapshot: snapshot([], { sttSeconds: 7200 - 10 }) }, 20)
    expect(month).toMatchObject({ limit: 'sttSecondsPerMonth', allowed: 7200, resetsAt: OCTOBER })
    expect(month?.message).toMatch(/120 minutes of Murmur transcription on the free plan are used up; more on Thu, Oct 1/)
  })
  ...
  it('applies Pro’s fair use: soft pauses formatting, hard stops transcription', () => {
    ...
    expect(checkFormatting(soft, true)).toEqual({
      ok: true,
      paused: { limit: 'fairUseSttSecondsPerMonth', used: 108_000, allowed: 108_000, exceeded: true, resetsAt: OCTOBER }
    })
    expect(checkFormatting(soft, false)).toMatchObject({
      ...
      refusal: {
        ...
        message: "Murmur's formatting model is paused until Thu, Oct 1: this month's 30 hours of fair use are used. Dictation continues with rule-based cleanup."
      }
    })
    ...
    expect(checkTranscription(hard, 1)).toMatchObject({
      ...
      allowed: 216_000,
      message: "This month's 60 hours of Murmur transcription, the Pro plan's fair-use limit, are used up; more on Thu, Oct 1."
    })
  ...
  it('caps formatting requests a day on the free tier and tokens a month everywhere', () => {
    const formats = checkFormatting({ ...free, snapshot: snapshot([bucket(TODAY, { formats: 12 })]) }, true)
    ...
    expect(checkFormatting({ ...free, snapshot: snapshot([bucket(TODAY, { dictations: 12 })]) }, true)).toEqual({ ok: true, paused: null })
    const tokens = checkFormatting({ ...pro, snapshot: snapshot([], { llmTokens: 25_000_000 }) }, true)
    expect(tokens).toMatchObject({ ok: false, refusal: { limit: 'llmTokensPerMonth', allowed: 25_000_000, resetsAt: OCTOBER } })
  })
  ...
  it('counts transcript words and builds the upgrade link', () => {
    expect(transcriptWords(JSON.stringify({ text: "ask not what your country can do for you, it's 2026" }), 'application/json')).toBe(11)
    ...
    expect(upgradeUrlFor({ MURMUR_SITE_URL: 'https://murmur.app/' }, 'free')).toBe('https://murmur.app/account?upgrade=yearly')
    expect(upgradeUrlFor({ MURMUR_SITE_URL: 'https://murmur.app' }, 'trial')).toBe('https://murmur.app/account?upgrade=yearly')
...
describe('gateway: free tier', () => {
  it('counts words from the transcript and stops at 500 a week with a structured error', async () => {
    ...
    expect(status.window).toEqual({ day: TODAY, weekStart: '2026-09-07', words: 500, sttSeconds: 5, dictationsToday: 1 })
    expect(status.meters[0]).toMatchObject({ limit: 'wordsPerWeek', used: 500, exceeded: true, resetsAt: Date.UTC(2026, 8, 16) })
    ...
    expect(await blocked.json()).toEqual({
      error: {
        ...
        code: 'quota_exceeded',
        message: "You've dictated the 500 words a week the free plan includes; more open up on Wed, Sep 16, or go unlimited with Pro.",
        limit: 'wordsPerWeek',
        plan: 'free',
        planState: 'free',
        used: 500,
  ...
  it('caps audio a week, dictations a day and the clip length', async () => {
    ...
    expect((await audio.json()).error).toMatchObject({ limit: 'sttSecondsPerWeek', used: 400, allowed: 420, resetsAt: Date.UTC(2026, 8, 19) })
    expect((await asAda.fetch('/v1/audio/transcriptions', await sttRequest(makeWav(15)))).status).toBe(200)

    const long = await asAda.fetch('/v1/audio/transcriptions', await sttRequest(makeWav(61)))
    expect(long.status).toBe(413)
    expect((await long.json()).error).toMatchObject({ code: 'clip_too_long', limit: 'maxClipSeconds', allowed: 60, resetsAt: null })
    ...
    expect((await day.json()).error).toMatchObject({ limit: 'dictationsPerDay', used: 12, allowed: 12, resetsAt: TOMORROW })
    expect(day.headers.get('retry-after')).toBe('43200')
    // Formatting has its own side of the daily cap.
    stubFetch(() => chatAnswer('Hello.'))
    expect((await asAda.fetch('/v1/format', formatRequest('hello there everyone'))).status).toBe(200)
  ...
  it('keeps the monthly backstops and the free request rate', async () => {
    ...
    expect((await month.json()).error).toMatchObject({ limit: 'sttSecondsPerMonth', allowed: 7200, resetsAt: OCTOBER })
    ...
    expect((await tokens.json()).error).toMatchObject({ limit: 'llmTokensPerMonth', allowed: 500_000 })
    await seedMonth(t, userId, { llmTokens: 0, windowStart: NOW - 1000, windowCount: PLANS.free.requestsPerMinute })
    const rate = await asAda.fetch('/v1/audio/transcriptions', await sttRequest(makeWav(1)))
    expect(rate.status).toBe(429)
    expect((await rate.json()).error).toMatchObject({ code: 'rate_limited', limit: 'requestsPerMinute', used: 20, allowed: 20, resetsAt: NOW + 59_000 })
...
describe('gateway: Pro fair use', () => {
  it('pauses the formatting model past 30 audio-hours and throttles, but keeps transcribing', async () => {
    ...
    expect((await asAda.fetch('/v1/audio/transcriptions', await sttRequest(makeWav(61)))).status).toBe(200)
    expect(calls).toHaveLength(1)
    ...
    const format = await asAda.fetch('/v1/format', formatRequest('um so the budget is one million dollars'))
    ...
    expect((await throttled.json()).error).toMatchObject({ limit: 'requestsPerMinute', allowed: 20 })
  ...
  it('applies the trial exactly like Pro', async () => {
    ...
    expect((await asAda.fetch('/v1/audio/transcriptions', await sttRequest(makeWav(120)))).status).toBe(200)
    const status = await asAda.query(api.inference.status, { day: TODAY })
    expect(status).toMatchObject({ plan: 'pro', planState: 'trial', upgradeUrl: `${SITE}/account?upgrade=yearly` })
...
describe('trial lifecycle', () => {
  it('starts every new account on a 14-day trial that a scheduled function ends', async () => {
    ...
    expect(await asAda.query(api.users.me, {})).toMatchObject({ planState: 'trial', trialEndsAt: NOW + TRIAL_MS })

    await t.finishAllScheduledFunctions(vi.runAllTimers)
    expect(Date.now()).toBeGreaterThanOrEqual(NOW + TRIAL_MS)
    expect(await asAda.query(api.users.me, {})).toMatchObject({ plan: 'free', planState: 'free', trialEndsAt: NOW + TRIAL_MS })
    const status = await asAda.query(api.inference.status, {})
    expect(status).toMatchObject({ plan: 'free', planState: 'free', limits: { maxClipSeconds: 60 } })
  })
  ...
  it('settles a stale trial on the next contact even if the scheduled job never ran', async () => {
    ...
    expect(long.status).toBe(413)
    expect((await long.json()).error).toMatchObject({ plan: 'free', planState: 'free', allowed: 60 })
  ...
  it('lets the operator restart a trial or set a state by hand', async () => {
    ...
    expect(again).toMatchObject({ plan: 'pro', planState: 'trial', trialEndsAt: NOW + 2 * TRIAL_MS + DAY_MS })
    expect(await t.mutation(internal.users.setPlan, { clerkId: 'user_ada', plan: 'pro' })).toMatchObject({ planState: 'pro' })
    expect(await t.mutation(internal.users.setPlan, { clerkId: 'user_ada', plan: 'free' })).toMatchObject({ planState: 'free', plan: 'free' })
  })
  ...
  it('backfills existing accounts with 14 days from deploy time, page by page', async () => {
    ...
    expect(await t.mutation(internal.entitlements.backfillTrials, {})).toEqual({ granted: 0, isDone: false })
    vi.advanceTimersByTime(1)
    await t.finishInProgressScheduledFunctions()
    ...
    expect(await t.withIdentity({ subject: 'legacy_3' }).query(api.users.me, {})).toMatchObject({ planState: 'pro' })
  })
})
...
describe('inference.status windows', () => {
  it('returns the rolling week, the day and every reset for the day the client passes', async () => {
    ...
    const status = await asAda.query(api.inference.status, { day: TODAY })
    expect(status.window).toEqual({ day: TODAY, weekStart: '2026-09-07', words: 160, sttSeconds: 84, dictationsToday: 3 })
    expect(status.meters).toEqual([
      ...
      { limit: 'llmTokensPerMonth', used: 5000, allowed: 500_000, exceeded: false, resetsAt: OCTOBER }
    ])
    ...
    expect(later.resets).toEqual({ day: Date.UTC(2026, 8, 16), week: Date.UTC(2026, 8, 19), month: OCTOBER })
